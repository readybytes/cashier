    ...
    $(document).ready(function () {
        ...
        $("input[name='stripe-payment-option-update']").change(function () {
            ...
            if(new_card == 1){
                ...
                $(form).parent().find("a.vod-add-card").show();
            } else{
                ...
                $(form).parent().find("a.vod-add-card").hide();
            }
        });
    });
</script>
...
@if(count($payment_details[$card_usage]))
    <div class="uk-alert uk-padding-remove">
        <div class="uk-form-controls">
            <label class="uk-margin-large-right"><input type="radio" value="old" data-new-card="0" data-card-usage="{{$card_usage}}" data-processor-type="{{$form_processor_type}}" name="stripe-payment-option-update" checked> Use Existing Card</label>
            <label class="uk-margin-large-left"><input type="radio" value="new" data-new-card="1" data-card-usage="{{$card_usage}}" data-processor-type="{{$form_processor_type}}" name="stripe-payment-option-update"> Add New Card</label>
        </div>
    </div>
@endif
...
<div class="uk-form-row">
    ...
    <div class="uk-form-controls">
        <input type="text" data-name="number" value="{{@$payment_details[$card_usage]["number"]}}" placeholder="Card Number" class="uk-form-width-medium" @if(count($payment_details[$card_usage])) readonly @endif>
    </div>
</div>
...
<input type='hidden' name='customer_id' value='{{@$payment_details[$card_usage]["customer_id"]}}' />
